...
class GradeSheetParser ( Parser ):
    ...
    class RContext(ParserRuleContext):
        ...
        def ID(self):
            return self.getToken(GradeSheetParser.ID, 0)

        def getRuleIndex(self):
            return GradeSheetParser.RULE_r
    ...
    def r(self):
        ...
        localctx = GradeSheetParser.RContext(self, self._ctx, self.state)
        self.enterRule(localctx, 0, self.RULE_r)
        try:
            self.enterOuterAlt(localctx, 1)
            self.state = 2
            self.match(GradeSheetParser.T__0)
            self.state = 3
            self.match(GradeSheetParser.ID)
